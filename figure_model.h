...
public:
    ...
    std::tuple<figure::Vertex*, int> get_vertices() const
    {
    ...
    std::tuple<float**, int>  get_normals() const
    {
        auto res = new float*[faces.size()];
        for (int i = 0; i < faces.size(); i++)
        {
            res[i] = new float[3];
            for (int j = 0; j < 3; j++)
            {
                res[i][j] = normals[faces[i][j].normal_index][j];
            }
        }
